...
let
  ...
  conditional_import =
    ...
    (
      ...
      args: {
        ...
        imports = builtins.map (conditional_import condition) (
          if realized ? imports then realized.imports else [ ]
        );
        # Move all of the top level configs into a config array and combine with
        # config values
        config = args.lib.mkIf (condition) (config // (if config ? config then config.config else { }));
      }
    );
  ...
  wrap =
    ...
    in
    (conditional_import enabled) module;
