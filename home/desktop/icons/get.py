...
codepoint_re = re.compile(r"content: \"\\([a-z0-9]{4,5})\";")
    ...
    with open(css_path) as css:
        for line in css:
            if m := class_re.match(line):
                css_line = next(css);
                codepoint = int(codepoint_re.match(css_line.strip()).group(1), 16)
...
with open(args.nix, 'w') as nixfile:
    ...
    for icon, codepoint in parse_css(args.css):
        nixfile.write(f"  {icon} = {{char = \"{chr(codepoint)}\"; codepoint = \"{codepoint}\"; icon = {args.dir}/share/nerd-icons/{codepoint:x}.svg; }};\n")
    nixfile.write("}\n")
